...
from ..utils import PretrainedMixin, maybe_use_tqdm
...
class Resample(nn.Module):
    ...
    def __init__(self, dim: int, mode: RESAMPLE_MODE_LITERAL | None = None) -> None:
        """
        :param dim: input dimension
    ...
    def forward(
        self,
        x: torch.Tensor,
        feat_cache: Sequence[str | torch.Tensor] | None = None,
        feat_idx: list[int] = [0],
    ) -> torch.Tensor:
        """
        :param x: input tensor [B, C, T, H, W]
...
class WanVAE(PretrainedMixin):
    ...
    def encode(
        self,
        x: torch.Tensor,
        scale: tuple[torch.Tensor, torch.Tensor] | tuple[float, float],
    ) -> torch.Tensor:
        """
        :param x: input tensor [B, C, T, H, W]
...
class WanVideoVAE(WanVAE, ModelMixin, ConfigMixin):
    ...
    def tiled_decode(
        ...
        for h, h_, w, w_ in maybe_use_tqdm(
            tasks, desc="VAE decoding", use_tqdm=use_tqdm
        ):
    ...
    def single_decode(
        self, hidden_states: torch.Tensor, device: torch.device, loop: bool
    ) -> torch.Tensor:
        """
        :param hidden_states: hidden states tensor [B, C, T, H, W]
        :param device: device
        ...
        for hidden_state in hidden_states:
            ...
            if tiled:
                video = self.tiled_decode(
                    hidden_state, device, tile_size, tile_stride, loop
                )
            else:
