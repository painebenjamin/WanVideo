...
class WanSelfAttention(nn.Module):
    ...
    def apply_rope(
        self, x: torch.Tensor, grid_sizes: torch.Tensor, freqs: torch.Tensor
    ) -> torch.Tensor:
        """
        :param x: The input tensor with shape [B, L, C].
...
def get_wan_crossattention_class(model_type: WAN_MODEL_TYPE_LITERAL) -> Type[nn.Module]:
    """
...
class WanAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm3 = (
            WanLayerNorm(dim=dim, eps=eps, elementwise_affine=True)
            if cross_attn_norm
            else nn.Identity()
        )
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    @amp.autocast("cuda", enabled=False)  # type: ignore[attr-defined,misc]
    def rope_params(
        self, max_seq_len: int, dim: int, theta: int = 10000
    ) -> torch.Tensor:
        """
    ...
    def sinusoidal_embedding_1d(self, dim: int, position: torch.Tensor) -> torch.Tensor:
        """
